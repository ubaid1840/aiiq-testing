...
import RetrievalHistory from "@/Components/production/RetrievalHistory";
...
export default function Page({ backendUrl, routes }) {
  ...
  async function fetchData(last) {
    try {
      const response = await getResult(backendUrl, last);
  ...
  async function fetchFilterData() {
    ...
    try {
      const response = await axios.post(`${backendUrl}/sessions/filter`, data);
      setFilterPress(true);
      if (response.data.length > 0) {
        response.data.forEach((obj) => {
          if (obj.hasOwnProperty("session_id")) {
