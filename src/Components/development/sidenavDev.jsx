...
import SearchQueryDev from "@/Components/development/searchQueryDev";
...
export default function Sidebar({ children, url, allowedRoutes, onReturn, currentPage }) {
  ...
  function getLinkItems() {
    if (pathname.toLowerCase().includes("stacked-dev") || pathname.toLowerCase().includes("mpa")) return LinkItemsDev;
    else return LinkItemsProd;
  }
    ...
    >
      ...
      <Box ml={{ base: 0, md: 60 }} p="4" display={"flex"}>
        ...
        {openSearch && (
          ...
          >
            {url && (
              <SearchQueryDev url={url} closeSearch={() => setOpenSearch(false)} />
            )}
          </Box>
        )}
      </Box>
    ...
    >
      <div>
        <Flex h="20" alignItems="center" mx="8" justifyContent="space-between" gap={2}>
          <Text fontSize="xl" fontFamily="monospace" fontWeight="bold" mb={0}>
            {`AIIQ Engine`}
          </Text>
          <Text fontSize="12px" fontFamily="monospace" mb={0}>
            {` ${version ? `v${version}` : ""}`}
          </Text>
          <CloseButton
