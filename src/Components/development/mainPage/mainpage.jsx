...
import Data from "@/Components/development/chatbot/Animation.json";
...
export default function MainPage({ backendUrl, allRoutes }) {
  ...
  async function handleEndSession(retry = false) {
    ...
    setHistorySessions();
    try {
      axios.get(`${backendUrl}/end-session`, {withCredentials : true}).then(async (response) => {
                ...
                <div className={`message-container-ai`}>
                  ...
                  <VStack align={"flex-start"} gap={0}>
                    <div className="message-ai" style={{ marginBottom: "0px" }}>
                      ...
                      <Wrap>
                      {historySessions?.data[index - 3]?.images &&
                        historySessions?.data[index - 3]?.images.length > 0 &&
                        historySessions?.data[index - 3]?.images.map(
                          (eachImage, imgind) => (
                            <WrapItem key={imgind} m={"5px"} p={3} borderRadius={'8px'} border={'1px solid'} borderColor={'#CCCCCC'}>
                              <Image
                              _hover={{cursor:'pointer', opacity:0.7}}
                                height={"150px"}
                                width={"150px"}
                                src={`data:image/jpeg;base64,${eachImage}`}
                                onClick={() => {
                                  setSelectedImage(
                                    `data:image/jpeg;base64,${eachImage}`
                                  );
                                  onOpenImageView();
                                }}
                              />
      ...
      >
        <HStack alignItems={"center"} gap={3}>
          {!sidebarHide && (
            <HStack gap={5}>
              {logoUrl && (
                <Image src={logoUrl} height={"50px"} ml={3} alt="logo" />
              )}
              <Icon
                ml={3}
              ...
              {historyLoading ? (
                ...
                sessions.length > 0 && (
                  <VStack gap={2}>
                    {sessions
                      .filter((eachSession) =>
                        eachSession?.headline?.toLowerCase()
