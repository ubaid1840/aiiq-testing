...
function RetrievalHistory({ session_id, url, thumbsFilter, exclude }) {
  ...
  useEffect(() => {
    const fetchChatHistory = async () => {
      try {
        const res = await axios.get(`${url}/sessions/${session_id}`, {withCredentials : true});
        ...
        >
          ...
          <VStack align={"flex-start"} gap={0}>
            ...
            <Wrap>
              {images &&
                images.length > 0 &&
                images.map((eachImage, imgind) => (
                  <WrapItem key={imgind} m={"10px"} borderRadius={'8px'} border={'1px solid'} borderColor={'#CCCCCC'} p={3}>
                    <Image
                    borderRadius={'8px'}
                    _hover={{cursor:'pointer', opacity:0.7}}
                      ...
                      onClick={() => {
                        setSelectedImage(`data:image/jpeg;base64,${eachImage}`);
                        onOpenImageView();
                      }}
                    />
                  </WrapItem>
